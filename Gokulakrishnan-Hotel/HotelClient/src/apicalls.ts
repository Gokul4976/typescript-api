...
export async function AddItemToCart(cartItem: Wishlist): Promise<string> {
    console.log(cartItem.stayingFrom,"stay");
...
export async function BuySingleItem(cartID: string, customerID: string): Promise<string> {
    ...
    if (!response.ok) {
        throw new Error('Failed to buy room');
    }
    return await response.text();
}
...
export async function purchaseAll(customerID: string) {
...
export async function CheckIsBooked(cart:Wishlist)
{
    // console.log(typeof(startdate)+'api');
    // console.log(startdate.toString());
    
    
    const response = await fetch(`http://localhost:5224/api/hotel/roomSelectionController/checkbooking/${cart.wishlistID}`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json'
        }
