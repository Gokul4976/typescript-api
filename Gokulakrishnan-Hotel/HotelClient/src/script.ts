...
async function PurchaseAllItems() {
    ...
    if (bookingStatus)
    {
        ...
        alert("some of the room is already booked")
    }
}
...
async function BuySingleCartItem(cart: Wishlist) {
    var isBooked=await APICALLS.CheckIsBooked(cart);
    console.log(isBooked,"isbook");
    if (isBooked){

        
        var orderConfirmation = await APICALLS.BuySingleItem(cart.wishlistID, currentCustomer.customerID);
        currentCustomer = <CustomerRegistration>await APICALLS.GetIndividualUser(currentCustomer.mailID, currentCustomer.password);
...
async function displayBillContent(booking: BookingDetails, purchases: RoomSelection[]) {
    ...
    for (let item of purchases) {
        console.log(item.stayingDateTo,"item1");
        ...
        row.innerHTML = `
            ...
            <td>${productDetail?.numberOfBeds}</td>
            <td>${item.stayingDateFrom}</td>
            <td>${item.stayingDateTo}</td>
            <td>$${item.price}</td>
        `;
