...
function BuySingleCartItem(cart) {
    return __awaiter(this, void 0, void 0, function* () {
        var isBooked = yield APICALLS.CheckIsBooked(cart);
        console.log(isBooked, "isbook");
        if (isBooked) {
            var orderConfirmation = yield APICALLS.BuySingleItem(cart.wishlistID, currentCustomer.customerID);
            currentCustomer = (yield APICALLS.GetIndividualUser(currentCustomer.mailID, currentCustomer.password));
...
function displayBillContent(booking, purchases) {
    ...
    `;
        for (let item of purchases) {
            ...
            <td>${productDetail === null || productDetail === void 0 ? void 0 : productDetail.numberOfBeds}</td>
